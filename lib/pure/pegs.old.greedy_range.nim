...
type
  PegKind* = enum
    ...
    pkGreedyRangeRepeat,## a#i or a#i,j where i and j are digits
  ...
  Peg* {.shallow.} = object ## type that represents a PEG
    ...
    of pkGreedyRangeRepeat:
      min, max: int
      daughters: seq[Peg] # = sons, but has a different name because you can't redifine a type attribute in a the case clause.
...
proc repeat*(a: Peg, min, max: int): Peg {. noSideEffect, rtl, extern: "npegs$1".} =
  result.kind = pkGreedyRangeRepeat
  result.min = min
  result.max = max
  result.daughters = @[a]
...
proc backrefIgnoreStyle*(index: range[1..MaxSubpatterns]): Peg {.
  ...
  result.index = index-1
    # Create a helper function to generate a pkRepeat Peg node. (which I do actually need)
...
proc rawMatch*(s: string, p: Peg, start: int, c: var Captures): int {.
  ...
  of pkGreedyRangeRepeat:
    ...
    var numMatches = 0
    var matchLen = 0
    while numMatches <= p.max + 1:
      # Optimization. If we have already matched too many times, no point in continuing to loop.
      var x = rawMatch(s, p.daughters[0], start+matchLen, c)
      if x >= 0:
        numMatches += 1
        matchLen += x
      if x <= 0: break
    if numMatches >= p.min and numMatches <= p.max:
      inc(result, matchLen)
  of pkGreedyRepChar:
    result = 0
    var ch = p.ch
...
type
  ...
  Token {.final.} = object  ## a token
    ...
    repeat: tuple[min, max: int]  ## if kind == tkHashBang
...
proc skip(c: var PegLexer) =
  ...
  while pos < c.buf.len:
    ...
    of '#':
      ...
      # since we are using # as a special character, make double hashbang, "##" mean a comment instead.
...
proc getHashBang(c: var PegLexer, tok: var Token) =
  var pos = c.bufpos + 1
  var buf = c.buf
  if buf[pos] in {'0'..'9'}:
    tok.kind = tkRepeat
    tok.repeat.min = 0
    while pos < c.buf.len and buf[pos] in {'0'..'9'}:
      tok.repeat.min = tok.repeat.min * 10 + ord(buf[pos]) - ord('0')
      inc(pos)
    if buf[pos] == ',':
      inc(pos)
      tok.repeat.max = 0
      while pos < c.buf.len and buf[pos] in {'0'..'9'}:
        tok.repeat.max = tok.repeat.max * 10 + ord(buf[pos]) - ord('0')
        inc(pos)
    else:
      tok.repeat.max = tok.repeat.min
  else:
    tok.kind = tkHashBang
    add(tok.literal, '#')
  c.bufpos = pos
...
proc primary(p: var PegParser): Peg =
  ...
  while true:
    ...
    of tkPlus:
      ...
      getTok(p)
    of tkRepeat:
      if p.tok.repeat.min < 0 or p.tok.repeat.max < 0 or p.tok.repeat.max < p.tok.repeat.min:
        pegError(p, "Invalid Ranged Repeat range: " & $p.tok.repeat.min & ", " & $p.tok.repeat.max)
      result = repeat(result, p.tok.repeat.min, p.tok.repeat.max)
      getTok(p)
    else: break
...
proc seqExpr(p: var PegParser): Peg =
  ...
  while true:
    ...
    of tkAmp, tkNot, tkAt, tkStringLit, tkCharSet, tkParLe, tkCurlyLe,
       ...
       tkHat, tkCurlyAt, tkRepeat, tkHashBang:
